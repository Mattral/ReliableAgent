"""Tests for ReliableOrchestrator, the convenience wrapper closing the
audit-identified gap between this project's API and the roadmap's
illustrative DX example (adr/0008).
"""
...
def test_run_with_string_task_matches_roadmap_dx():
    ...
    try:
        result = orchestrator.run(task="Add 2 and 3", max_steps=20)
...
def test_run_with_prebuilt_task_object():
    orchestrator = ReliableOrchestrator(
        tools=_tools(), llm_client=MockLLMClient(responses=[_plan()])
    )
    try:
        result = orchestrator.run(Task(description="Add 2 and 3", max_steps=10, max_replans=1))
        assert result.final_state == OrchestratorState.COMPLETED
    finally:
        orchestrator.shutdown()
...
def test_checkpointing_false_uses_in_memory_backend():
    orchestrator = ReliableOrchestrator(
        tools=_tools(), llm_client=MockLLMClient(responses=[_plan()]), enable_checkpointing=False
    )
    try:
        assert isinstance(orchestrator._orchestrator.memory, InMemoryBackend)
    finally:
        orchestrator.shutdown()
...
def test_checkpointing_true_uses_file_backend():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            assert isinstance(orchestrator._orchestrator.memory, FileMemoryBackend)
        finally:
            orchestrator.shutdown()
...
def test_observability_false_uses_in_memory_sink():
    orchestrator = ReliableOrchestrator(
        tools=_tools(), llm_client=MockLLMClient(responses=[_plan()]), enable_observability=False
    )
    try:
        assert isinstance(orchestrator._orchestrator.sink, InMemorySink)
    finally:
        orchestrator.shutdown()
...
def test_observability_true_uses_console_sink():
    orchestrator = ReliableOrchestrator(
        tools=_tools(), llm_client=MockLLMClient(responses=[_plan()]), enable_observability=True
    )
    try:
        assert isinstance(orchestrator._orchestrator.sink, ConsoleSink)
    finally:
        orchestrator.shutdown()


def test_no_model_no_client_defaults_to_mock():
    orchestrator = ReliableOrchestrator(tools=_tools())
    try:
        assert isinstance(orchestrator._orchestrator.planner._llm_client, MockLLMClient)
    finally:
        orchestrator.shutdown()
...
def test_explicit_client_takes_precedence_over_model():
    ...
    orchestrator = ReliableOrchestrator(
        tools=_tools(), model="claude-sonnet-4-6", llm_client=client
    )
    try:
        assert orchestrator._orchestrator.planner._llm_client is client
    finally:
        orchestrator.shutdown()
...
def test_resume_delegates_correctly():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            result = orchestrator.run(task="Add 2 and 3")
            run_id = result.run_id

            orchestrator_2 = ReliableOrchestrator(
                tools=_tools(),
...
def test_context_manager_shuts_down_automatically():
    client = MockLLMClient(responses=[_plan()])
    with ReliableOrchestrator(tools=_tools(), llm_client=client) as orchestrator:
        result = orchestrator.run(task="Add 2 and 3")
        assert result.final_state == OrchestratorState.COMPLETED
